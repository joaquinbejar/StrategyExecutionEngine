...
impl MAStrategy {
    ...
    pub fn get_signal(&self) -> Option<Side> {
        ...
        // 计算当前的短期MA和长期MA
        let short_ma =
            prices_vec.iter().rev().take(self.short_period).sum::<f64>() / self.short_period as f64;
        ...
        let prev_prices: Vec<f64> = prices_vec
            .iter()
            .take(prices_vec.len() - 1)
            .cloned()
            .collect();

        // 计算前一个时间点的短期MA和长期MA
        let prev_short_ma = prev_prices
            .iter()
            .rev()
